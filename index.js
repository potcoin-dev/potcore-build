 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`potcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`potcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'potcore-' + name : 'potcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/potcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external potcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:potcore-lib -o potcore-lib.js';
    }
